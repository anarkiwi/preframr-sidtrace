...
| `SDCU`  | per-state-cell **UPDATE data-flow** summary (same entry layout as `SDDF`, keyed by the cell ADDRESS in the `pc` field): the backward slice of the STORE/RMW that DEFINED the cell — the update generator `cell' = f(cell, C, K, …)`. Emitted for the **transitive closure** of the generator DAG: the `SDDF` state-cell leaves SEED the set, then it is closed under each included cell's own update-slice leaves — so a generator that feeds ANOTHER generator (a global LFO/counter cell driving a per-voice freq cell), not a SID register directly, is emitted too. Output-agnostic: the host recovers structure (global-vs-voice, phase, recurrence family) from the WHOLE DAG, the emulator assuming none of it. The trailing value sequence carries the cell's mid-call generator-state stream (for Berlekamp–Massey). O(state cells), flat vs frames. |
