...
| `STSQ`  | inter-frame **STATE-CELL sample sequence**, keyed by RAM address, for every cell in the generator-DAG closure (see `SDCU`): `(addr u16, flags u8 [bit0=holdsToEnd, bit1=wide], _pad u8, totalFrames u32, firstSeenFrame u32, nSamples u16, then nSamples × sample u8)`. The bounded (≤`M=512`/cell) per-play-call value sequence Stage C fits the recurrence over (Daikon-style relation / Berlekamp–Massey). `holdsToEnd`: the constant-stride recurrence implied by the prefix kept holding to the end of the capture (a cheap in-emulator running check). O(state cells × M), flat once the window exceeds M and all state cells are discovered. |
| `SDCU`  | per-state-cell **UPDATE data-flow** summary, keyed by the cell ADDRESS — the backward slice of the STORE/RMW that DEFINED the cell — the update generator `cell' = f(cell, C, K, …)`. **Branch structure (SDST version 2):** the update is split by STORE SITE into guarded single-branch **variants** so a branch-conditioned generator (triangle PW/vibrato sweep, table-jump) is recovered losslessly rather than folded into a `{ADC,SBC}` union. Per-cell layout: `cell u16, nVariants u16`, then per variant an `SDDF`-shaped value entry (the `pc` slot = the variant's **store PC**, value slice recorded ONCE so `op_seq` is the true single-frame chain) followed by `nGuards u8` and, per guard, `(cmpOp u8, sense u8, immediate u8, flags u8 [bit0=unresolved], nLeaves u16, then nLeaves × leaf)`. `cmpOp` (`CmpOp`: EQ/NE/LT/GE/MI/PL/BIT_SET/BIT_CLR/VS/VC/UNRESOLVED) is the predicate that holds when the store fires; `sense` is the branch's taken(1)/fall-through(0) side from the executed-PC trajectory; the guard `leaves` are the tested operand's backward slice. A cell with one variant and zero guards is exactly the old straight-line SDCU (back-compatible). Emitted for the **transitive closure** of the generator DAG: the `SDDF` state-cell leaves SEED the set, then it is closed under each variant's value AND **guard** leaves — so a toggle/divider cell a guard tests (and a generator feeding another generator) is emitted too. The trailing per-variant value sequence carries the cell's mid-call generator-state stream (for Berlekamp–Massey). O(code sites × variants × guard-depth), flat vs frames. |
| `SDIR`  | **FAITHFUL generator-IR** (the lossless successor to the `SDDF`/`SDCU` summary; design `generator-ir-design.md` §4). Where `SDCU` emits an unordered op-ENVELOPE + a leaf union, `SDIR` emits the EXACT straight-line program each observed variant ran: an **ordered SSA body** (one node per executed ALU/load, in EXECUTION ORDER recovered from `RetiredInstr.ringSeq`) with fully-resolved operands — `Imm` / `Mem(base, mode, index_cell, klass)` / `Prior(cell)` (the recurrence self-edge, named not guessed) / `Node(id)` (an earlier node's result) — plus the evaluable guard chain (reused from `SDCU`), the post-settle `init` seed, and the bytes of any `Mem(TABLE)` operand. A body STOPS at persistent generator cells (emitted as `Mem`/`Prior` operands), so each cell's multi-term arithmetic lives in its own body, joined by operand edges; Python interprets it forward for byte-exact replay. **ADDITIVE** — every other section is byte-unchanged. Per-generator layout: `cell u16, destWidth u8, init u8, nVariants u8, nTables u16`, then per table `(base u16, stride u8, length u16, length × byte u8)`, then per variant `(storePc u16, storeNode u16, nGuards u8, guards…, nNodes u16, nodes…)`; each node is `(op u8, carryIn u8, nArgs u8)` then per arg a tagged operand (`Imm`: `i32`; `Mem`: `base u16, mode u8, indexCell u16, width u8, klass u8, value u8`; `Prior`: `cell u16, value u8`; `Node`: `id u16`). |
...
| `VEVT`  | **PER-VOICE NOTE TIMELINE**: the post-settle note schedule per SID voice. A gate edge (control reg $D404/$D40B/$D412 bit0 change) is a note ONSET (gate→1, carrying the voice's current 16-bit frequency = the note pitch) or RELEASE (gate→0); the emulator disambiguates note boundaries the raw register stream leaves implicit. `n_voices u32`; then per voice with events `(voice u8, _pad u8, _pad u16, n_events u32)` then `n_events × (frame u32, freq u16, ctrl u8, gate u8)` (`frame` = play-call index relative to settle). The host maps `freq` to an A440 12-TET grid index. Bounded per voice by `VE_CAP`. |
...
**Backward compatibility.** `SDIR` (and earlier `SETL`/`SSTR`) is appended
additively before the
`END\0` terminator; every prior section (`ACMP`/`SNAP`/`SIDW`/`IDXR`/`SDDF`/
`STSQ`/`SDCU`/`IDXS`/`PWLK`/`RELO`/`SDAC`/`DIGI`/`TMPO`/`IWLK`/`SETL`/`SSTR`/`VEVT`) is byte-unchanged,
